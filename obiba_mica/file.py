...
class StoreTrueFileAction(FileAction):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        super().__call__(parser, namespace, self.const, option_string=option_string)
...
class FileService:
    ...
    def create(self, path, name):
        ...
        return self.__make_request().post().resource(self.FILES_WS).content_type_json().content(json.dumps(dict(id="", fileName=".", path="/".join([MicaFile(path).path, name])))).send()
    ...
    def upload(self, file, upload):
        ...
        temp_file.update(dict(fileName=os.path.basename(fileName), justUploaded=True, path=MicaFile(file).path))

        return self.__make_request().post().resource(self.FILES_WS).content_type_json().content(json.dumps(temp_file)).send()
