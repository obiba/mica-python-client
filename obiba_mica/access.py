...
class AccessService:
    ...
    def add_access(self, id, type: str, subject: str, noFile: bool = True):
        ...
        """
        uri = UriBuilder(self._get_resource_path(id)).query("type", type.upper()).query("principal", subject).query("file", str(noFile).lower()).build()
    ...
    def delete_access(self, id, type: str, subject: str):
        ...
        :param subject - associated user/group
        """
        uri = UriBuilder(self._get_resource_path(id)).query("type", type.upper()).query("principal", subject).build()
    ...
    def validate_args(cls, args):
        ...
        if not args.list:
            ...
            if not args.type or args.type.upper() not in AccessService.SUBJECT_TYPES:
                raise Exception(f"Valid subject types are: {', '.join(AccessService.SUBJECT_TYPES).lower()}")
...
class ProjectAccessService(AccessService):
    ...
    def add_arguments(cls, parser):
        super().add_permission_arguments(parser, True)
...
class FileAccessService(AccessService):
    ...
    def add_arguments(cls, parser):
        ...
        """
        super().add_permission_arguments(parser, False)
