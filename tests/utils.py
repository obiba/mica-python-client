...
class Utils:
    ...
    def make_client(server=None, user=None, password=None):
        return MicaClient.buildWithAuthentication(
            server=Utils.SERVER if server is None else server, user=Utils.USER if user is None else user, password=Utils.PASSWORD if password is None else password, otp=None
        )
    ...
    def parse_arg_values(parser=None, server=None, user=None, password=None, params=[]):
        ...
        argv = ["test", "--mica", Utils.SERVER if server is None else server, "--user", Utils.USER if user is None else user, "--password", Utils.PASSWORD if password is None else password]
